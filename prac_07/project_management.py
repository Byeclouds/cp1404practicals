...
def update_project(projects):
    ...
    for i in range(len(projects)):
        print(
            f"{i} {projects[i].name}, start: {projects[i].start_date.strftime('%d/%m/%Y')}, "
            f"priority {projects[i].priority}, estimate: ${projects[i].cost_estimate:,.2f}, "
            f"completion: {projects[i].completion_percentage}%")

    project_number = int(input("Project choice: "))
    if 0 <= project_number < len(projects):
        index = project_number
        print(
            f"{projects[index].name}, start: {projects[index].start_date.strftime('%d/%m/%Y')}, "
            f"priority {projects[index].priority}, estimate: ${projects[index].cost_estimate:,.2f}, "
            f"completion: {projects[index].completion_percentage}%")
